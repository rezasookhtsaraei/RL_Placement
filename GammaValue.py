...
class DiabetesDAGEnv:
    ...
    def _generate_random_dag_with_priority_and_deadline(self):
        ...
        if priority == 1:
            deadline = np.random.uniform(1.5, 5.0)
        elif priority == 2:
            deadline = np.random.uniform(1.5, 10.0)
        else:
            deadline = np.random.uniform(1.5, 15.0)
        G.add_node(0,
...
class PPO:
    def __init__(self, state_dim, action_dim, lr=3e-4, gamma=0.9, clip_epsilon=0.2, entropy_coef=0.1):
...
if __name__ == "__main__":
    ...
    all_results = {}
    print(f"\n{'='*80}")
    print(f"COMPARING DIFFERENT GAMMA VALUES (clip_epsilon=0.1)")
    print(f"{'='*80}")
    for gamma_val in gamma_values:
        print(f"\n{'='*80}")
        print(f"Training with gamma = {gamma_val}, clip_epsilon=0.1")
        print(f"{'='*80}")
        ...
        agent = PPO(state_dim, action_dim, lr=3e-4, gamma=gamma_val, clip_epsilon=0.1, entropy_coef=0.1)
        print(f"Starting training with {num_episodes} episodes...")
        start_time = time.time()
        results = train_ppo_quick(env, agent, num_episodes=num_episodes)
        end_time = time.time()
        print(f"Training with gamma = {gamma_val} completed in {end_time - start_time:.2f} seconds")
        all_results[gamma_val] = results
    plt.figure(figsize=(15, 8))
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b']
    line_styles = ['-', '--', '-.', ':']
    final_values = {}
    for i, (gamma_val, results) in enumerate(all_results.items()):
        ...
        final_values[gamma_val] = final_avg
        line_style = line_styles[i % len(line_styles)]
        plt.plot(range(len(rewards)), rewards,
                 label=f'γ={gamma_val} (Final: {final_avg:.2f})',
                 color=colors[i % len(colors)],
                 linestyle=line_style,
                 linewidth=1.5,
                 alpha=0.8)
    plt.title('Reward Convergence for Different Gamma Values (clip_epsilon=0.1)\nRaw Reward Values (No Moving Average)',
    ...
    plt.figtext(0.5, 0.01,
                f'Raw reward values for all {num_episodes} episodes | Parameters: {num_dags} DAGs per episode, clip_epsilon=0.2',
                ha='center', fontsize=10, style='italic')
    best_gamma = max(final_values.items(), key=lambda x: x[1])
    best_text = f'Best: γ={best_gamma[0]} (Avg Reward: {best_gamma[1]:.2f})'
    ...
    plt.figtext(0.15, 0.94, f'Gamma γ Comparison (ε=0.2)',
                fontsize=11, fontweight='bold', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    plt.tight_layout(rect=[0, 0.05, 1, 0.98])
    plt.savefig('gamma_raw_rewards_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('gamma_raw_rewards_zoomed.png', dpi=300, bbox_inches='tight')
    plt.show()
    print(f"\n{'='*100}")
    print("SUMMARY RESULTS FOR DIFFERENT GAMMA VALUES")
    print(f"{'='*100}")
    print(f"{'Gamma γ':<10} {'Final Avg Reward':<18} {'Min Reward':<15} {'Max Reward':<15} {'Avg DAGs':<12} {'Avg Violations':<15}")
    print(f"{'-'*100}")
    for gamma_val, results in all_results.items():
        rewards, dags_completed, violations, backup_rates, response_times, load_deviations, reliability_scores, cloud_usage_rates, priority_rewards = results
        if len(rewards) >= 20:
        ...
        if gamma_val == best_gamma[0]:
            print(f"{gamma_val:<10.3f} \033[1;32m{final_avg_reward:<18.2f}\033[0m {min_reward:<15.2f} {max_reward:<15.2f} {final_avg_dags:<12.2f} {final_avg_violations:<15.2f}")
        else:
            print(f"{gamma_val:<10.3f} {final_avg_reward:<18.2f} {min_reward:<15.2f} {max_reward:<15.2f} {final_avg_dags:<12.2f} {final_avg_violations:<15.2f}")
    print(f"\n{'='*100}")
    print("REWARD VARIABILITY ANALYSIS")
    print(f"{'='*100}")
    print(f"{'Gamma γ':<10} {'Std Dev':<15} {'CV (%)':<15} {'Q1':<15} {'Median':<15} {'Q3':<15}")
    print(f"{'-'*100}")
    for gamma_val, results in all_results.items():
        rewards, _, _, _, _, _, _, _, _ = results
        ...
        if gamma_val == best_gamma[0]:
            print(f"{gamma_val:<10.3f} \033[1;32m{std_dev:<15.2f}\033[0m {cv:<15.1f} {q1:<15.2f} {median:<15.2f} {q3:<15.2f}")
        else:
            print(f"{gamma_val:<10.3f} {std_dev:<15.2f} {cv:<15.1f} {q1:<15.2f} {median:<15.2f} {q3:<15.2f}")
    print(f"\n{'='*100}")
    print("CONCLUSION")
    print(f"{'='*100}")
    print(f"• Best performing gamma value: \033[1;32mγ={best_gamma[0]}\033[0m")
    print(f"• Average reward for best value: \033[1;32m{best_gamma[1]:.2f}\033[0m")
    ...
    for gamma_val, results in all_results.items():
        ...
        print(f"• γ={gamma_val}: {trend} ({improvement:+.1f}% change)")
    
    print(f"\nSaving raw reward data to files...")
    for gamma_val, results in all_results.items():
        rewards, _, _, _, _, _, _, _, _ = results
        filename = f"rewards_gamma_{gamma_val}.txt"
        np.savetxt(filename, rewards, fmt='%.4f')
        print(f"  • Saved {len(rewards)} reward values for γ={gamma_val} to {filename}")