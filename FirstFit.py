...
def run_first_fit(env):
    print(f"\n{'='*80}")
    print(f"Starting First Fit Algorithm")
    print(f"{'='*80}")
    ...
    while not done:
        action = first_fit_allocation(env)
    ...
    print(f"\nFirst Fit Algorithm completed in {end_time - start_time:.2f} seconds")

    print(f"\n{'='*80}")
    print(f"Evaluating First Fit Algorithm on All DAGs")
    print(f"{'='*80}")
    ...
    print(f"\n{'='*80}")
    print(f"FINAL REPORT - First Fit Algorithm")
    print(f"{'='*80}")
...
if __name__ == "__main__":
    ...
    total_reward = run_first_fit(env)
